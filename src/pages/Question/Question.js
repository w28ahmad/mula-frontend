...
const Question = () => {
  ...
  const submitHandler = async (e) => {
    ...
    data["questionData"]["diagram"] = await saveDiagram(
      questionDiagram,
      "questionData",
      "diagram"
    );
    data["solutionData"]["diagram"] = await saveDiagram(
      solutionDiagram,
      "solutionData",
      "diagram"
    );
  ...
  return (
    <div style={{ display: "flex" }}>
      <div>
        <button style={{ margin: "10px" }} type="button" onClick={addQuestion}>
          Add Question
        </button>
        <div className="sidebar">
          <ul>
            {menuItems.map((item) => (
              <li
                className={item === parseInt(currId) ? "selected" : ""}
                style={{
                  display: "flex",
                  alignItems: "center",
            ...
            {Object.keys(data).map((key) => (
              <div key={key}>
                ...
                {Object.keys(data[key]).map((subKey) => (
                  ...
                  >
                    {subKey}:
                    {(key === "questionData" || key === "solutionData") && (
                      <Markdown>
                        {data[key][subKey].replace(/\\\\/g, "\\")}
                      </Markdown>
                    )}
                    {questionDiagram &&
                      key === "questionData" &&
